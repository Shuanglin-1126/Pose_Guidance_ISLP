...
class GCNModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.pool = nn.AdaptiveAvgPool1d(self.prefix_tokens)
    ...
    def forward(self, x: torch.Tensor, is_train: bool = True, wo_condition=False) -> torch.Tensor:
        ...
        if not wo_condition:
            ...
            x = self.fc(self.pool(x).permute(0, 2, 1))

            if is_train and use_dropout:
                drop_ids = torch.rand(B, device=x.device) < self.dropout_prob
                x = torch.where(drop_ids[:, None, None].repeat(1, self.prefix_tokens, self.hidden_size),
                                self.embedding_null[None, None, :],
                                x)  # x.shape=(6,8,1152), self.embedding_null.shape=(1152,), drop_ids.shape=(6,)

        else:
            x = self.embedding_null[None, None].repeat(B, self.prefix_tokens, 1)  # x.shape=(B,C)
