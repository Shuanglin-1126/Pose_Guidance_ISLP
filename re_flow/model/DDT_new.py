...
from re_flow.model.GCN_new import GCNModel
...
class DiTwDDTHead(nn.Module):
    def __init__(
        ...
        GCN_cfg['prefix_tokens'] = prefix_tokens
        ...
        self.prefix_tokens = prefix_tokens
        self.reference_pool = nn.AdaptiveAvgPool1d(1)
        self.pose_fc = nn.Linear(hidden_size[0], hidden_size[-1])
    ...
    def forward(self, x, t, joint, reference_image, s=None, is_sample=False):
        ...
        reference = self.reference_pool(reference.permute(0, 2, 1)).squeeze()  # B prefix_tokens C
        c = nn.functional.silu(t + reference)
        if s is None:
            s = self.s_embedder(x)
            if self.use_pos_embed:
                s = s + self.pos_embed
            # print(f"t shape: {t.shape}, y shape: {y.shape}, c shape: {c.shape}, s shape: {s.shape}, pos_embed shape: {self.pos_embed.shape}")
            s = torch.cat((y, s), dim=1)  # B (prefix_tokens + L) C
        ...
        x = torch.cat((self.pose_fc(y), x), dim=1)
