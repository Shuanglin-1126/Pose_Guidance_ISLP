...
class VisionRotaryEmbeddingFast(nn.Module):
    ...
    def forward(self, t):
        ...
        pre_t, lat_t = t[..., :self.prefix_tokens, :], t[..., self.prefix_tokens:, :]
        lat_t = lat_t * freqs_cos + rotate_half(lat_t) * freqs_sin
        return torch.cat((pre_t, lat_t), dim=-2)
...
class NormAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_LORA:
            self.qkv_LORA = LoRALinear(dim, dim * 3, rank=LORA_rank, bias=qkv_bias)
        ...
        if use_LORA:
            self.proj_LORA = LoRALinear(dim, dim, rank=LORA_rank, alpha=LORA_alpha)
    ...
    def forward(self, x: torch.Tensor, rope=None) -> torch.Tensor:
        ...
        if self.use_LORA:
            qkv = qkv + self.qkv_LORA(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
