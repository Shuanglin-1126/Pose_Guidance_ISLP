...
from re_flow.dataset import Image_Pose_Dataset, ImageScale, PoseNormalize, TransformersToTensor
...
def get_first_video_id_by_gloss(json_path, target_gloss):
    """
    根据 gloss 查找对应第一条 instances 的 video_id
    :param json_path: wlasl100_info.json 文件路径
    :param target_gloss: 要查找的单词，如 book / drink
    :return: 匹配到的第一个 video_id，没找到返回 None
    """
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    for item in data:
        if item.get("gloss") == target_gloss:
            instances = item.get("instances", [])
            if instances:
                return instances[0].get("video_id")
    return None
...
def main(args):
    ...
    file_path = r'F:\SLRdataset\WLASL\information\WLASL_v0.3.json'
    vid = get_first_video_id_by_gloss(file_path, args.input)
    npz_path = os.path.join(r'F:\SLRdataset\WLASL\body_keypoint_133', f'{vid}.npz')
    dataset_pose, dataset_pose_score = _load_pose(npz_path)

    dataset_pose = torch.flip(dataset_pose, dims=[-1])
    joint_ = get_transform(dataset_pose, dataset_pose_score, (256, 256))[0:16, ...]

    using_cfg = guidance_scale > 1.0
    zs = torch.randn(dataset_config['num_frames'], *latent_size, device=device, dtype=torch.float32)

    img_path = os.path.join(r'F:\SLRdataset\WLASL\origin_frame', f'{vid}\\0001.jpg')
    img_ref = load_image(img_path)
    sample_path = r"\system\sample_result"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, default='book')
